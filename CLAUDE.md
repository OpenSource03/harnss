...
- `SplitTopRowItem.tsx` — renders a single item (chat pane or tool column) in the top row; extracted from AppLayout
- `SplitBottomToolIsland.tsx` — renders a single tool island in the split-view bottom dock; extracted from AppLayout
- `SplitPaneToolStrip.tsx` — per-pane tool picker strip (mirrors the single-pane ToolPicker strip)
